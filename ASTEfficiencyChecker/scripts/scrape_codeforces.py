...
def fetch_problemset(timeout: int = 30) -> dict[str, Any]:
    """
    Call the problemset.problems endpoint and return the raw parsed JSON payload.
    """
...
def extract_problems(payload: dict[str, Any]) -> list[dict[str, Any]]:
    """
    Extract and normalize the fields we care about from the raw API payload.
    """
    try:
    ...
    if skipped:
        logger.warning("Skipped %d problem(s) missing required fields.", skipped)
...
def save_dataset(problems: list[dict[str, Any]], output_path: str) -> None:
    """
    Write `problems` to `output_path` as pretty-printed JSON.
    """
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not problems:
            logger.error("No problems extracted from the API response; aborting.")
            return 1
